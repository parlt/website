...
function getLocalnetURI() {
  return EnvHelper.localnetURI;
}
...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  // const [chainID, setChainID] = useState(Number(process.env.REACT_APP_CHAIN_ID));
  let REACT_APP_CHAIN_ID = 80001;
  const [chainID, setChainID] = useState(REACT_APP_CHAIN_ID);
  ...
  const _checkNetwork = (otherChainID: number): boolean => {
    if (chainID !== otherChainID) {
      ...
      if (otherChainID === 1 || otherChainID === 4 || otherChainID == 80001 || otherChainID == 1987 || otherChainID === 1337) {
        setChainID(otherChainID);
        otherChainID === 1 ? setUri(getMainnetURI()) : otherChainID === 4 || otherChainID === 80001 ? setUri(getTestnetURI()) : setUri(getLocalnetURI());
        return true;
      }
      return false;
    }
  ...
  const connect = useCallback(async () => {
    ...
    let chainId = await connectedProvider.getNetwork().then(network => network.chainId);
